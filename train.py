...
def run():
    ...
    input_size = 200  # 输入特征数，即数据的维度（如温度、湿度等）
    ...
    for epoch in range(epochs):
        ...
        for X, y in val_loader:
            with torch.no_grad():  # 关闭梯度计算
                ...
                all_pred += pred.size()[0]
                correct = pred.eq(torch.argmax(y, dim=1)).sum().item()  # 统计预测正确的数量
                total_correct += correct  # 累加预测正确的数量
        accuracy = total_correct / all_pred * 100  # 计算准确率并乘以100转换为百分比
        
        print(f"epoch: {epoch}, loss: {loss}, val_acc: {accuracy}%", epoch, loss, accuracy)
